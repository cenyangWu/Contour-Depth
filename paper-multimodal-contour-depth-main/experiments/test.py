...
# Perform Kernel PCA
kpca = KernelPCA(n_components=8, kernel='rbf', gamma=1e-2)
...
# Calculate the center of contours_kpca
center = np.mean(contours_kpca, axis=0)

# Compute distances of each element to the center
distances = np.linalg.norm(contours_kpca - center, axis=1)

# Sort elements based on their distance to the center
sorted_indices = np.argsort(distances)
sorted_contours_kpca = contours_kpca[sorted_indices]
...
for i, (index, distance) in enumerate(zip(sorted_indices, sorted_distances)):
    print(f"Element {index}: Distance to center = {distance}")

# Visualize sorted elements in Kernel PCA space
plt.figure(figsize=(8, 8))
plt.scatter(sorted_contours_kpca[:, 0], sorted_contours_kpca[:, 1], c=sorted_cluster_labels, cmap='viridis')
plt.xlabel('Kernel PCA Component 1')
plt.ylabel('Kernel PCA Component 2')
plt.title('Sorted Contours in Kernel PCA Space')
plt.colorbar(label='Cluster Label')
plt.show()

# Generate a colormap based on sorted distances
colormap = cm.viridis(np.linspace(0, 1, len(sorted_indices)))

# Plot all contours, color-coded by their sorted order based on distance to center
fig, ax = plt.subplots(figsize=(8, 8))
for i, idx in enumerate(sorted_indices):
    contour = sampled_contours[idx]
    color = colormap[i]  # Assign color based on the sorted order
    ax.plot(contour[:, 0], contour[:, 1], color=color, linewidth=1)

# Invert y-axis for correct orientation
ax.invert_yaxis()
ax.set_title('Contours Color-Coded by Distance to Center (Sorted Order)')
ax.set_xlabel('X')
ax.set_ylabel('Y')

# Add a colorbar for the distance order
sm = plt.cm.ScalarMappable(cmap='viridis', norm=plt.Normalize(vmin=0, vmax=len(sorted_indices)))
sm.set_array([])  # Dummy array for colorbar
fig.colorbar(sm, ax=ax, label='Distance Order')
